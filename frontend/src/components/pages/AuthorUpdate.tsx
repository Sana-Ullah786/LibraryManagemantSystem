...
import { AuthorDetails, ErrorObject } from "../../CustomTypes";
...
function AuthorUpdate(props: Props): ReactElement {
    ...
    client
      ...
      .then((Author) => {
        if (Author) {
          ...
          setDateOfBirth(Author.birth_date);
          setDateOfDeath(Author.death_date);
          setLoaded(true);
        }
      })
      .catch((error) => {
  ...
  const onSubmit: SubmitHandler<AuthorDetails> = (data) => {
    client.PutAuthor(id, cleanData(data)).then((response) => {
